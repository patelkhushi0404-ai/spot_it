...
const statusConfig = {
  ...
  inprogress: { label: 'In Progress', color: '#ea580c', bg: '#fff7ed', border: '#fed7aa' },
...
const ReportsPage = () => {
  ...
  const handleAssign = async (reportId) => {
    if (!assignWorker) { toast.error('Please select a worker'); return; }
    setAssigning(true);
    try {
      await API.put('/reports/' + reportId + '/assign', { workerId: assignWorker });
    ...
    <AdminLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{reports.length} total reports</p>
        </div>
        <button
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        {filters.map((f) => (
          ...
          >
            {f.label} {f.value !== 'all' && `(${reports.filter(r => r.status === f.value).length})`}
          </button>
        ))}
      </div>
          ...
          {filteredReports.map((report) => {
            ...
            return (
              <div
                ...
                onClick={() => { setSelectedReport(report); setAssignWorker(''); }}
              >
                {/* Image */}
                <div className="h-40 overflow-hidden relative bg-gray-100">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentElement.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:2.5rem;color:#9ca3af">📸</div>';
                    }}
                  />
                  <div
                ...
                <div className="p-4">
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <p className="text-orange-500 text-xs truncate">{report.location?.address || 'Unknown'}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400 text-xs">{report.user?.name || 'Unknown'}</span>
                    </div>
          ...
          >
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-gray-700 text-sm">{selectedReport.location?.address || 'Unknown'}</p>
              </div>

              {/* Description */}
              <div>
                <p className="text-gray-400 text-xs mb-1 font-medium">Description</p>
                <p className="text-gray-700 text-sm">{selectedReport.description}</p>
              </div>

              {/* Reporter */}
              <div>
                <p className="text-gray-400 text-xs mb-1 font-medium">Reported By</p>
                <p className="text-gray-700 text-sm">{selectedReport.user?.name} ({selectedReport.user?.email})</p>
              </div>

              {/* Assigned Worker */}
              {selectedReport.assignedWorker?.name && (
                <div>
                  <p className="text-gray-400 text-xs mb-1 font-medium">Assigned Worker</p>
                  <p className="text-gray-700 text-sm">{selectedReport.assignedWorker.name} — {selectedReport.assignedWorker.phone}</p>
                </div>
              )}

              {/* Date */}
              <div>
                <p className="text-gray-400 text-xs mb-1 font-medium">Submitted</p>
                <p className="text-gray-700 text-sm">{new Date(selectedReport.createdAt).toLocaleString('en-IN')}</p>
              </div>
              ...
              <div className="space-y-3 pt-2 border-t border-gray-100">
                {(selectedReport.status === 'pending' || selectedReport.status === 'assigned') && (
                  <div>
                    <p className="text-gray-400 text-xs mb-2 font-medium">Assign Worker</p>
                    {workers.length === 0 ? (
                      <p className="text-gray-400 text-xs">No workers available. Add workers first.</p>
                    ) : (
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          {workers.map((w) => (
                            <option key={w._id} value={w._id}>{w.name} — {w.area}</option>
                          ))}
                        </select>
                        <button
                ...
                {selectedReport.status === 'assigned' && (
                  <button
                    onClick={() => handleStatusUpdate(selectedReport._id, 'inprogress')}
                ...
                {(selectedReport.status === 'assigned' || selectedReport.status === 'inprogress') && (
                  <button
