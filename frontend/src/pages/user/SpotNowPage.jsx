    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          setForm((prev) => ({ ...prev, lat: latitude, lng: longitude, address: latitude + ', ' + longitude }));
          toast.success('Location detected!');
        } finally {
    ...
    >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Capture waste, submit report, earn points!</p>
        </div>
          ...
          {[1, 2, 3].map((s) => (
            <React.Fragment key={s}>
              <div
                ...
                style={{
                  background: step >= s ? 'linear-gradient(135deg, #fb923c, #ea580c)' : 'rgba(30,41,59,0.8)',
                  border: step >= s ? '1px solid #fb923c' : '1px solid rgba(100,116,139,0.4)',
                  color: step >= s ? '#fff' : '#64748b',
                  boxShadow: step >= s ? '0 4px 15px rgba(249,115,22,0.3)' : 'none',
                }}
              >
                {step > s ? '✓' : s}
              </div>
              {s < 3 && (
                <div
                  ...
                  style={{
                    background: step > s ? 'linear-gradient(90deg, #fb923c, #4ade80)' : 'rgba(100,116,139,0.3)',
                  }}
                ></div>
              )}
            </React.Fragment>
        ...
        <div className="flex justify-center gap-12 mb-8 text-xs">
          <span style={{ color: step >= 1 ? '#fb923c' : '#64748b', fontWeight: step >= 1 ? '600' : '400' }}>
            Capture
          </span>
          <span style={{ color: step >= 2 ? '#fb923c' : '#64748b', fontWeight: step >= 2 ? '600' : '400' }}>
            Details
          </span>
          <span style={{ color: step >= 3 ? '#fb923c' : '#64748b', fontWeight: step >= 3 ? '600' : '400' }}>
            Done
          </span>
        </div>
        ...
        {step === 1 && (
          ...
          >
            <div className="p-5" style={{ borderBottom: '1px solid rgba(100,116,139,0.2)' }}>
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-4 left-4 w-8 h-8" style={{ borderTop: '2px solid #fb923c', borderLeft: '2px solid #fb923c', borderRadius: '4px 0 0 0' }}></div>
                <div className="absolute top-4 right-4 w-8 h-8" style={{ borderTop: '2px solid #fb923c', borderRight: '2px solid #fb923c', borderRadius: '0 4px 0 0' }}></div>
                <div className="absolute bottom-4 left-4 w-8 h-8" style={{ borderBottom: '2px solid #fb923c', borderLeft: '2px solid #fb923c', borderRadius: '0 0 0 4px' }}></div>
                <div className="absolute bottom-4 right-4 w-8 h-8" style={{ borderBottom: '2px solid #fb923c', borderRight: '2px solid #fb923c', borderRadius: '0 0 4px 0' }}></div>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {locationLoading ? (
                        <div className="w-5 h-5 border-2 rounded-full animate-spin" style={{ borderColor: 'rgba(74,222,128,0.3)', borderTopColor: '#4ade80' }}></div>
                      ) : 'GPS'}
                    </button>
                  </div>
                  {form.lat && form.lng && (
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl font-bold text-white text-lg transition-all duration-200"
                  style={{
                    background: loading ? 'rgba(249,115,22,0.5)' : 'linear-gradient(135deg, #fb923c, #ea580c)',
        ...
        {step === 3 && (
          ...
          >
            ...
            <p className="text-slate-300 mb-2">Your waste report has been submitted successfully.</p>
